...
class Evaluation:    
  def __init__(self, synthetic, real, warn=False):
  ...
  def switch_warnings(self):
    warnings.warn = warnings.warn if self.warn else self._warn
  ...
  def table_evaluator_metrics(self,target_column):  
    table_evaluator = TableEvaluator(self.real,self.synthetic)
    table_evaluator.evaluate(target_col=target_column)
  ...
  def table(self,target_column,cat_target,keys):        
    ...
    print("(3/7) Cont. Kullback–Leibler divergence...",resemblance_3)
    resemblance_4=DiscreteKLDivergence.compute(self.real,self.synthetic)
    print("(4/7) Discrete Kullback–Leibler divergence...",resemblance_4)
